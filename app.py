...
@app.route('/user_results/<job_id>', methods = ['POST', 'GET'])
def user_results(job_id): # route for user results site. Takes Job ID
    ...
    # bss_ress_filt = bss_ress[["UniProt_ResNum", "alignment_column", "abs_norm_shenkin", "oddsratio", "pvalue", "AA", "RSA", "SS"]] # filtering columns
    # bss_ress_filt.columns = cc_new # changing column names
    ...
    print(bss_ress.binding_sites.unique().tolist()[0], type(bss_ress.binding_sites.unique().tolist()[0]))

    print(first_site, type(first_site))
    ...
    print(sorted([f for f in os.listdir(job_supp_cifs_dir) if f.endswith(".cif")])[0])
    ...
    # seg_stats = load_pickle(os.path.join(STATS_FOLDER, "{}_stats.pkl".format("P00517_1"))) # {'P00517': {'1': {'strucs': 84, 'ligs': 103, 'bss': 8, 'seqs': 4783, 'v_seqs': 495}}}
    ...
    return render_template(
        ...
        simple_pdbs = simple_pdbs_full_path, assembly_pdb_ids = assembly_pdb_ids, prot_atoms_rep = prot_atoms_struc
    )
