...
def transform_lines2(defattr_in, opened_chimX, loaded_3dmol):
    """
        ...
        for line in file:
            ...
            if match:
                ...
                if opened_chimX[int(key)] in loaded_3dmol:
                    ...
                    data.append((mod_id, chain, int(resi), int(p_bs)))
                else:
...
def process_model_order():
    data = request.json
    loaded_order = data['modelOrder']
    segment_name = data['segmentName']
    cxc_in = os.path.join(DATA_FOLDER, "P00517", "{}_ALL_inf_average_0.5.cxc".format(segment_name))
    attr_in =  os.path.join(DATA_FOLDER, "P00517", "{}_ALL_inf_average_0.5.defattr".format(segment_name))

    model_order = extract_open_files(cxc_in)

    # print(attr_in, model_order, loaded_order)

    result_tuples, bs_ids = transform_lines2(attr_in, model_order, loaded_order)
